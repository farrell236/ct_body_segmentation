# 3D residual unet architecture based on 2D
...
def ResidualBlock(width):
    def apply(x):
        ...
        if input_width == width:
            ...
            residual = tf.keras.layers.Conv3D(width, kernel_size=1)(x)
        x = tf.keras.layers.BatchNormalization()(x)
        x = tf.keras.layers.Conv3D(
            width, kernel_size=3, padding="same", activation=tf.keras.activations.swish
        )(x)
        x = tf.keras.layers.Conv3D(width, kernel_size=3, padding="same")(x)
...
def get_network(input_shape, widths, block_depth, n_classes=10, activation='softmax'):
    ...
    x = tf.keras.layers.Conv3D(widths[0], kernel_size=1)(image_input)
    ...
    x = tf.keras.layers.Conv3D(n_classes, 1, activation=activation)(x)
...
if __name__ == '__main__':
    ...
    unet = get_network((None, 256, 256, 1), widths, block_depth, n_classes=10)

    a=1

    import numpy as np
    x = np.random.rand(2, 128, 256, 256, 1).astype('float32')
    out = unet(x)
